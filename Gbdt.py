...
class RuleEnsemble():
    ...
    def __init__(self,
        ...
        """
        self.rules_0 = set()  #每个评估器中的第一棵树
        self.rules_1 = set()  #每个评估器中的第二棵树
        self.rules_2 = set()  #每个评估器中的第三棵树
        self.rules_3 = set()  #每个评估器中的第四棵树
        self.result = result  #gdbt在训练集上的预测结果
        ## TODO: Move this out of __init__
        self._extract_rules()  #提取规则
        self.rules_0 = list(self.rules_0)
        self.rules_1 = list(self.rules_1)
        self.rules_2 = list(self.rules_2)
        self.rules_3 = list(self.rules_3)

    def _extract_rules(self):
        """Recursively extract rules from each tree in the ensemble
        """
        count = 1 # 记录提取规则的树，树的序号从1开始
        number = 0 #记录进入了几次循环
        for tree in self.tree_list:
            number += 1
            # tree_index = self.result[number]  #提取每次四颗树中的第几棵树
            for tree_index in range(0,4):
                count += tree_index
                # number += 1
                # tree[0].tree_ 每一个树对象
                """
                ...
                8、n_node_samples：size类型，代表了训练时落入到该节点的样本总数。显然，父节点的n_node_samples将等于其左右子节点的n_node_samples之和
                """
                # if number == len(self.tree_list):
                #     dot_data = export_graphviz(tree[0][tree_index], out_file=None,
                #                                feature_names=self.feature_names,filled=True, rounded=True)
                #     graph = pydotplus.graph_from_dot_data(dot_data)
                #     graph.write_png("out"+str(tree_index)+".png")
                """
                这里应该提取满足的该变量预测种类的树的规则
                """
                rulesFromTree = extract_rules_from_tree(tree[0][tree_index].tree_, count,feature_names=self.feature_names)
                if tree_index == 0:
                    self.rules_0.update(rulesFromTree)
                elif tree_index == 1:
                    self.rules_1.update(rulesFromTree)
                elif tree_index == 2:
                    self.rules_2.update(rulesFromTree)
                else:
                    self.rules_3.update(rulesFromTree)
                a = 1
            # print(self.rules)
    ...
    def transform(self, X0,X1,X2,X3):
        """Transform dataset.
        Parameters
        ----------
        X:      array-like matrix, shape=(n_samples, n_features)
        ...
        """
        rule0_list = list(self.rules_0)
        rule1_list = list(self.rules_1)
        rule2_list = list(self.rules_2)
        rule3_list = list(self.rules_3)
        ...
        rule0 = np.array([rule.transform(X0) for rule in rule0_list]).T
        rule1 = np.array([rule.transform(X1) for rule in rule1_list]).T
        rule2 = np.array([rule.transform(X2) for rule in rule2_list]).T
        rule3 = np.array([rule.transform(X3) for rule in rule3_list]).T
...
class Tree(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y, feature_names=None):
        ...
        self.flag_0 = [] #标记预测正确的 0 标签的训练数据索引
        self.flag_1 = []
        self.flag_2 = []
        self.flag_3 = []
        id = -1
        for GT in y:
            ...
            if predictTrain[id] == GT: #预测值正确
                if GT == 0:#此样本为0标签，则flag_0为id(样本索引)值
        ...
        """
        self.X0_rules,self.X1_rules,self.X2_rules,self.X3_rules = self.rule_ensemble.transform(self.predictTrue0,self.predictTrue1,self.predictTrue2,self.predictTrue3)
        # for i in range(self.X0_rules.shape[0]):#遍历每一个正确预测为标签0的样本
        #     print("样本%s决策路径为:"%(i),end='')
        #     decide_path = ''
        #     for j in range(self.X0_rules.shape[1]):#遍历每一条规则，看此样本是否满足，并且输出
        #         if self.X0_rules[i][j] == 1:
        #             decide_path = decide_path + str(self.rule_ensemble.rules_0[j])+"&"
        ...
        样本决策路径
        """
        # self.n_nodes_ = [t[0].tree_.node_count for t in self.tree_generator.estimators_]
        # self.children_left_ = [t[0].tree_.children_left for t in self.tree_generator.estimators_]
        # self.children_right_ = [t[0].tree_.children_right for t in self.tree_generator.estimators_]
        # self.feature_ = [t[0].tree_.feature for t in self.tree_generator.estimators_]
        # self.threshold_ = [t[0].tree_.threshold for t in self.tree_generator.estimators_]
        """
        save trees  多分类是多棵树
        """
        ...
        # for dt in tree_list:
        #     if count < 5:
        #         dot_data = tree.export_graphviz(dt[0], out_file=None,
        #                                         feature_names=feature_names,
        #                                         filled=True, rounded=True,
        #                                         class_names=[0,1])
        #         graph = pydotplus.graph_from_dot_data(dot_data)
        #         # graph.write_png('./tree_pic/'+str(count )+"_0"+"DTtree.png")
