...
def extractRules0(path, allRules):
    ...
    for ruleList in Rules:
        ...
        for decpath in ruleList:
            ...
            if (cou < len(ruleList) - 1):
                ...
                decidePath += str(decpath)
        Tree.sort()
        allR = AllRules0(decidePath, Tree)  # 每个样本记录缩减后重要性高的路径
...
if __name__ == '__main__':
    ...
    temp = allRules.copy()
    temp.insert(0, 'index')
    X0Rules = gbdt_tree.X_Others_rules
    X0Index = gbdt_tree.flag_others  # 正确预测为样本1的索引

    """
    规则写入csv
    """
    with open(r'./satisfy_rules0.csv',"a",newline='') as file:
        writer = csv.writer(file)
        writer.writerow(temp)
        for index,item in enumerate(X0Rules):
            value = item.tolist()
            value.insert(0,X0Index[index])
            writer.writerow(value)
        file.close()
    print("标签0样本规则提取完成 ！")
    exRules = extractRules0(r'./satisfy_rules0.csv', allRules)
    """
    将规则按照样本满足数量由上到下排序
    """
    for i in range(len(exRules) - 1):
        for j in range(i + 1, len(exRules)):
            if (exRules[i].num < exRules[j].num):
                ...
                exRules[i] = exRules[j]
                exRules[j] = val
    joblib.dump(exRules, 'rules0.pkl')
    rulesMergeRank = []
    rulesMergeRank_str = []
    for oneRules in exRules:
        ...
        if (mergeRules in rulesMergeRank_str):  # 不同样本规则重复
            ...
            ruleMergeObject = AllRules0(mergeRules, origin_fromTree)
    ...
    joblib.dump(rulesMergeRank, 'rules0.pkl')